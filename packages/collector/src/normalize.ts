...
function toPlayerState(entry: PlayerListEntry, active?: ActivePlayer): PlayerState {
  return {
    ...
    currentHealth: active?.currentHealth ?? 0,
    maxHealth: active?.maxHealth ?? 0,
    currentResource: active?.currentResource ?? 0,
    maxResource: active?.maxResource ?? 0,
    resourceType: active?.resourceType ?? "NONE",
